locals {
  ...
  vpc_id = try(aws_vpc_ipv4_cidr_block_association.cidr_association[0].vpc_id, aws_vpc.vpc[0].id, "")
...
resource "aws_vpc_ipv4_cidr_block_association" "cidr_association" {
...
resource "aws_default_route_table" "default" {
  ...
  dynamic "route" {
    ...
    content {
      ...
      gateway_id                = aws_internet_gateway.internet_gateway[0].id #lookup(route.value, "gateway_id", null)
...
resource "aws_nat_gateway" "this" {
  ...
  depends_on = [aws_internet_gateway.internet_gateway, aws_subnet.public, aws_eip.nat]
}
