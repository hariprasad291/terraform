...
# locals {
#   endpoints = { for k, v in var.endpoints : k => v if var.enable_endpoints && try(v.enable_endpoints, true) }
# }
...
#   vpc_id            = local.vpc_id
#   service_name      = data.aws_vpc_endpoint_service.this[each.key].service
